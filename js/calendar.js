...
var popUpCal = {
    ...
    drawCalendar: function () {
		...
		html += '<th colspan="7" class="calendarHeader">'+getMonthName(popUpCal.selectedMonth)+' ';
		
		html += '&nbsp;&nbsp;&nbsp;<select id="cmbAno" >';
		
		
		for ( x=1970; x<2020; x++ )
		{ html += '<option value="' + x + '" ';
		  if ( x == popUpCal.selectedYear )
		  { html += ' selected '; }
		  html += '>'+ x + "</option>";  
		}
		...
		$('#cmbAno').change( function() {
			popUpCal.selectedYear =  $('#cmbAno').val();
            popUpCal.drawCalendar(); 
		});
