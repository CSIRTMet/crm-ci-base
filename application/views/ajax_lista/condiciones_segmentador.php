...
function Hidedatefield(){
	...
	$("#opciones_sub_respuesta").hide("fast");
	$("#opciones_estado_registro").hide("fast");
...
function Datatype(){
	...
	if(type_array[0].search('date')!=-1){
		...
		var url='<?php echo base_url() ?>index.php/lista_gestion/desplegar_valor_condiciones_filtro/';
		
		switch(type_array[1]) //trae el nombre del campo a comparar.
		{
			
			case 'estado_registro': 
					$.post(url, { 
						'operacion':'estado_registro'
					}, function(data) {
						$('#text_number').html(data);
					});
					break;
				 
			case 'mejor_gestion':   
					$.post(url, { 
						'operacion':'mejor_gestion'
					}, function(data) {
						$('#text_number').html(data);
					});
					break;
			...
			default: 
			        ...
			        $("#text_number").html('<input type="text" name="valor" id="valor" value="" maxlength="20" size="30">'); 
					break;
		}
