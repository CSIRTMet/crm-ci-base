...
#div_telefono , #div_direccion, #div_compromiso, #div_email {
...
// Selects one or more elements to assign a simpletip to
		...
		$("#select_sub_respuesta").change(function(){
				...
				else if (id == "11" || id == "29" || id == "30" || id == "31" || id =="32" || id == "33" || id =="34" || id =="35"  || id =="36" || id =="37" || id =="38" || id =="48"  || id =="52"  )	
				{ //acepta del 1 al 9 
...
function enviar_formulario() {
       ...
       $("#Grabar").attr("disabled","disabled");
		...
		$.post("<?php echo base_url() ?>index.php/gestion/ingresar_gestion_cobranza", { 
			 ...
			 //hfc cobranza
			 ,recibio_carta: $("#select_recibio_carta").val()
...
//valida el monto del pago total
function vPagoTotal_abono(){

	var tipo_pago = $("#select_tpago").val();
	var deuda = $("#txt_monto_documento").val();
	var pago = 0;
	if(tipo_pago == 1 || tipo_pago == 2){
		pago = $("#txt_monto_pago").val();
		if(deuda != "") deuda = Math.round(parseInt(deuda)); 
		if(pago == "" || pago == 0){
			...
			if(tipo_pago == 1){
				if(pago < deuda){
					alert("En esta modalidad el pago no puede ser menor que la deuda");
					
				}else{
					//alert("|"+deuda+"|"+pago+"|");
					if(deuda < pago){
						alert("El pago no puede ser mayor que la deuda"); 
					
					}
				}
			}else{
				if(tipo_pago == 2){
					if(deuda > 0 && (pago <= 0 || pago > deuda)){
						if(deuda < pago) alert("El abono no puede ser mayor que la deuda"); 
						else alert("En esta modalidad se debe ingresar el monto abonado");						
					}
				}
			}
		}
...
function vAbono(){
	...
	/*
	var deuda = document.getElementById("monto_total").value;
	alert(deuda);
	*/
	if(deuda != "") deuda = Math.round(parseInt(deuda)); 
	if(abono != "") abono = Math.round(parseInt(abono));

	if(deuda > 0 && (abono <= 0 || abono > deuda)){
		if(deuda < abono) alert("El abono no puede ser mayor que la deuda"); 
		else alert("En esta modalidad se debe ingresar el monto abonado");
...
function vMontoCuota(){
	var id = $("#select_ncuota").val();
	if (id != "0"){
		var deuda = $("#txt_monto_documento").val();
		var pie = $("#txt_monto_pie").val();
		var ncuota = parseInt(id);		
		if(deuda != "") deuda =  Math.round(parseInt(deuda)); 
		if(pie != "") pie =  Math.round(parseInt(pie));
		else pie = 0;
		if(pie > 0){
			var vcuota =  Math.round((deuda-pie)/ncuota);
			$("#txt_monto_cuota").val(vcuota);
			
		}else{
			alert("El valor del pie no puede ser 0");
...
function dame_el_50_porciento(){
	var deuda = $("#txt_monto_documento").val();
	if(deuda != "") deuda =  Math.round(parseInt(deuda));
	else deuda = 0; 
	var cincuenta_por_ciento = Math.round((deuda*50)/100)
	$("#txt_monto_pie").val(cincuenta_por_ciento);
}
function validarPago() {
	...
	if (id != "0"){
		var deuda = $("#txt_monto_documento").val();
		var pie = $("#txt_monto_pie").val();
		var vcuota = $("#txt_monto_cuota").val();
		var ncuota = parseInt(id);		
		if(deuda != "") deuda =  Math.round(parseInt(deuda));
		else deuda = 0; 
		if(deuda == 0){
			alert("La deuda no puede estar en 0");
		}		
		if(vcuota != "") vcuota = Math.round(parseInt(vcuota)); 
		else vcouta = 0;
		
		if(vcuota == 0){
			alert("El valor de la cuota no puede ser 0");
		}
		if(pie != "") pie =  Math.round(parseInt(pie));
		else pie = 0;
		var dTotal = (vcuota * ncuota)+pie;
		if(pie > 0 && vcuota > 0 && deuda > 0){
			var treinta_por_ciento = Math.round((deuda*30)/100)
			if(pie >= treinta_por_ciento){
				$("#txt_monto_cuota").val(vcuota);
				if(dTotal > deuda || dTotal < deuda){
					alert("Los montos no coinciden, se modificara el pie");
					var vpie = deuda -(vcuota * ncuota);
					$("#txt_monto_pie").val(vpie);
				}
			}else{
				alert("El pie no puede ser menor al 30 por ciento");
			}
		}else{
			alert("El valor del pie no puede ser 0");
		}
...
//Muestra el posible monto de la cuota según la cantidad de coutas seleccionada
$("#select_ncuota").change(function(){
	var id = $("#select_ncuota").val();
	if (id != "0"){
		var deuda = $("#txt_monto_documento").val();
		var pie = $("#txt_monto_pie").val();
...
<div id="div_deuda"  style="display:">
  ...
  <table width="100%" border="0" cellpadding="1" cellspacing="1" class="x-grid-page">
	...
	<tr>
      ...
      <td nowrap="nowrap" class="x-grid3-row-over"><?php echo $dias_mora; 
	if ($this->session->userdata('campana_id_campana') == 12 || $this->session->userdata('campana_id_campana') == 9){
		$recibio_carta = $deuda->recibio_carta;
	}
  ...
  <table width="100%" border="0" cellpadding="1" cellspacing="1" class="x-progress-inner" >
    ...
    <tr>
      ...
      <td><select name="select_respuesta" id="select_respuesta" tabindex="11">
       ...
       <td>
	  <?php 
	  if ($this->session->userdata('campana_id_campana') == 12 || $this->session->userdata('campana_id_campana') == 9){ //claro hfcc cobranza
	  ?>
	  <select name="select_recibio_carta" id="select_recibio_carta" >
        <option value="0" <?php
			if($recibio_carta == 0) echo "selected";
			?> >* Recibi&oacute; carta? * </option>
		<option value="1"
		<?php
			if($recibio_carta == 1) echo "selected";
			?>>SI</option>
		<option value="2"
		<?php
			if($recibio_carta == 2) echo "selected";
			?>>NO</option>
      </select>
	   <?php 
	   }else{
...
<tr>
    ...
    <td width="60%" class="ui-layout-resizer-closed-hover"><input type="text" id="txt_monto_pago" maxlength="7" value="" class="numbersOnly"  name="txt_monto_pago"></td>
  </tr>
</table>
</div>
...
<tr>
    ...
    <td width="60%" class="ui-layout-resizer-closed-hover"><input type="text" id="txt_monto_pie" name="txt_monto_pie" maxlength="7" value="" value="" class="numbersOnly"  ></td>
  </tr>
<tr>
...
<tr>
    ...
    <td width="60%" class="ui-layout-resizer-closed-hover"><input type="text" maxlength="7" id="txt_monto_cuota" name="txt_monto_cuota" value="" value="" class="numbersOnly"  ></td>
  </tr>
</table>
</div>
