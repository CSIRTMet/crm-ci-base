...
$db2 = $this->load->database('default2', TRUE);
	...
	if ($tipo_campana == 1) { //cobranza
				...
				$consulta =  "select * from campana_cliente_mkt where id_campana = ".$id_campana." and id_cliente = ".$id_cliente."  Limit 1; ";
			}
...
function insert_entry()
	...
	function buscar_registros($columna_a_buscar,$valor_a_buscar,$tipo_campana,$id_lista)
  {
   ...
   $sql ="
		...
		(select g.id_sub_respuesta from gestion_mkt  g where g.estado_gestion = 2 and g.id_cliente = lc.id_cliente order by fecha_termino desc limit 1)
		) AS ultima_gestion
   
   		,(select u.nombre from usuario u where u.id_usuario = 
		(select g.id_usuario from gestion_mkt  g where g.estado_gestion = 2 and g.id_cliente = lc.id_cliente order by fecha_termino desc limit 1)
		) AS usuario
	...
	function get_compromiso_cliente($id_cliente, $id_campana){
	   $query_aux = "select id_compromiso,
	   (select numero_documento from deuda where deuda.id_deuda = c.id_deuda ) as numero_documento, 
	   id_gestion, id_cliente, id_deuda,monto,fecha_compromiso,lugar_de_pago,
	   tipo_de_pago,forma_de_pago, numero_cuotas, pie, monto_cuota, (select fecha_de_pago from deuda where deuda.id_deuda = c.id_deuda ) as fecha_de_pago
	   from compromiso c where id_cliente = ".$id_cliente." 
	 ...
	 function get_compromiso_usuario($id_usuario, $id_campana){
	   $query_aux = "select 
		u.nombre as nombre_usuario, 
		 c.id_compromiso, 
		 (select numero_documento from deuda where deuda.id_deuda = c.id_deuda ) as numero_documento,
		 (select rut from deuda where deuda.id_deuda = c.id_deuda ) as rut,
		 c.id_gestion, 
		 c.id_cliente,
		 c.id_deuda, 
		 c.monto, 
		 ...
		 c.monto_cuota,
		 c.glosa,
		 (select fecha_de_pago from deuda where deuda.id_deuda = c.id_deuda ) as fecha_de_pago
	   	from compromiso c ,
		usuario u where  u.id_usuario = ".$id_usuario." 
	  ...
	  function get_venta_usuario($id_usuario, $id_campana){
	   $query_aux = "select 
		gmkt.id_gestion, 
		cmkt.rut as rut_cliente,
		cmkt.dv ,
		cmkt.nombre as nombre_cliente,
		sr.id_sub_respuesta,
		...
		gmkt.glosa,
		gmkt.fecha_termino,
		sr.nombre as resultado,
		u.nombre as usuario
		from gestion_mkt as gmkt 
		inner join cliente_mkt as cmkt on (gmkt.id_cliente = cmkt.id_cliente)
		...
		where u.id_usuario = ".$id_usuario." and gmkt.id_sub_respuesta in (29,30,31,32,33,34,35)
		and id_campana= ".$id_campana." 
	   and (year(gmkt.fecha_termino) = year(now()) and month(gmkt.fecha_termino) = month(now()))
	   order by gmkt.fecha_termino desc ";
	   $query = $this->db->query($query_aux);
	   return $query;
	   //return $query_aux;
	 }
	 function get_gestiones_usuario($id_usuario, $id_campana, $tipo_campana){
		if($tipo_campana == 1){
			...
			$query_aux = "SELECT COUNT( gestion_mkt.id_nivel4 ) AS cantidad, (SELECT nombre FROM nivel4 n4 WHERE n4.id_nivel4 = gestion_mkt.id_nivel4) AS sub_respuesta
			...
			group by  gestion_mkt.id_nivel4";
		}
	   $query = $this->db->query($query_aux);
	   return $query;
	 }
